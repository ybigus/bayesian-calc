...
orig_users = 1 + col1.number_input('Visitors', step=1, value=1000, key='orig_users')
orig_converted = 1 + col1.slider('Converted', min_value=0, max_value=orig_users, value=100, key='orig_converted')
#col1.number_input('Converted', step=1, value=100, key='orig_converted')
col2.header('Variation B:')
variation_users = 1 + col2.number_input('Visitors', step=1, value=1005, key='variation_users')
variation_converted = 1 + col2.slider('Converted', min_value=0, key='variation_converted', max_value=variation_users, value=101)
...
st.title('Probability B > A: %s%%' % int(round(p*100)))
st.title('Probability A > B: %s%%' % int(round((1-p)*100)))